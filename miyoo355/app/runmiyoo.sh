...
if [ ${miyoo_fw_update} -eq 1 ] ; then
    TMP_DESC_FILE=/tmp/sdcard_fw_desc.txt
    TARGET_MODEL=miyoo355
    TARGET_VERSION_FILE=/usr/miyoo/version
    FIRMWARE_FILE=$miyoo_fw_dir/miyoo355_fw.img

    echo "============= check fw version ============"
    dd if=$FIRMWARE_FILE of=$TMP_DESC_FILE bs=512 count=1
    str=`cat $TMP_DESC_FILE`
    oldversion=`cat $TARGET_VERSION_FILE`
    echo "current verison[$oldversion]"

    versionline=`sed -n '2p' $TMP_DESC_FILE`
    version=${versionline:8}
    echo "new version[$version]"

    modelline=`sed -n '1p' $TMP_DESC_FILE`
    model=${modelline:6}
    echo "model[$model]"

    if [ "$model" = "$TARGET_MODEL" ];then
        echo "get $TARGET_MODEL firmware"
